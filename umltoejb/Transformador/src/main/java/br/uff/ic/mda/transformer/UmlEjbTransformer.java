...
public class UmlEjbTransformer extends Transformer<UmlDomain, EjbDomain, UmlEjbDomain> {
    ...
    private String findEjbDataTypeBasedonUmlId(String umlId) throws Exception {
        ...
        // Regra Minha
        result = processQueryResult(intermediateDomain.query("UMLDataTypeToEJBDataType.allInstances()->select(a | a.dataType->exists(dt : DataType | dt = " + umlId + "))"));
    ...
    private String findEjbDataClassBasedonUmlId(String umlId) throws Exception {
        ...
        // Regra 3
        result = processQueryResult(intermediateDomain.query("UMLClassToEJBEntityComponent.allInstances()->select(a | a.class->includes(" + umlId + "))"));
        if (result != null && result.length > 0 && !"".equals(result[0])) {
            return processQueryResult(intermediateDomain.query(result[0] + ".dataClass"))[0];
        }
        // Regra 4
        result = processQueryResult(intermediateDomain.query("UMLClassToEJBDataClass.allInstances()->select(a | a.class->includes(" + umlId + "))"));
        if (result != null && result.length > 0 && !"".equals(result[0])) {
            return processQueryResult(intermediateDomain.query(result[0] + ".ejbDataClass"))[0];
        }
        // Regra 6
        result = processQueryResult(intermediateDomain.query("UMLAssociationClassToEJBDataClass.allInstances()->select(a | a.associationClass->includes(" + umlId + "))"));
        if (result != null && result.length > 0 && !"".equals(result[0])) {
            return processQueryResult(intermediateDomain.query(result[0] + ".ejbDataClass"))[0];
        }
    ...
    private String findEjbKeyClassBasedonUmlId(String umlId) throws Exception {
        ...
        // Regra 1
        result = processQueryResult(intermediateDomain.query("UMLClassToEJBKeyClass.allInstances()->select(a | a.class->exists(c : Class | c = " + umlId + "))"));
        if (result != null && result.length > 0 && !"".equals(result[0])) {
            return processQueryResult(intermediateDomain.query(result[0] + ".keyClass"))[0];
        }
        // Regra 2
        result = processQueryResult(intermediateDomain.query("UMLAssociationClassToEJBKeyClass.allInstances()->select(a | a.associationClass->exists(ac : AssociationClass | ac = " + umlId + "))"));
        if (result != null && result.length > 0 && !"".equals(result[0])) {
            return processQueryResult(intermediateDomain.query(result[0] + ".keyClass"))[0];
        }
    ...
    private String findEJBAssociationEndBasedonUmlAssociationEnd(String associationEndId) throws Exception {
        ...
        // Regra 8
        result = processQueryResult(intermediateDomain.query("UMLAssociationEndToEJBDataEndusingRule8.allInstances()->select(a | a.associationEnd->exists(ae : AssociationEnd | ae = " + associationEndId + "))"));
        if (result != null && result.length > 0 && !"".equals(result[0])) {
            return processQueryResult(intermediateDomain.query(result[0] + ".ejbAssociationEnd"))[0];
        }
        // Regra 9
        result = processQueryResult(intermediateDomain.query("UMLAssociationEndToEJBDataEndusingRule9.allInstances()->select(a | a.associationEnd->exists(ae : AssociationEnd | ae = " + associationEndId + "))"));
        if (result != null && result.length > 0 && !"".equals(result[0])) {
            return processQueryResult(intermediateDomain.query(result[0] + ".ejbAssociationEnd"))[0];
        }
        // Regra 10
        result = processQueryResult(intermediateDomain.query("UMLAssociationEndEmEJBAssociationusingRule10.allInstances()->select(a | a.associationEnd->exists(ae : AssociationEnd | ae = " + associationEndId + "))"));
        if (result != null && result.length > 0 && !"".equals(result[0])) {
            return processQueryResult(intermediateDomain.query(result[0] + ".ejbAssociationEnd2"))[0];
//            return tratarResultadoQuery(juncao.query(resultado[0] + ".ejbAssociationEnd1"))[0];
        }
        // Regra 11
        result = processQueryResult(intermediateDomain.query("UMLAssociationEndEmEJBAssociationusingRule11.allInstances()->select(a | a.associationEnd->exists(ae : AssociationEnd | ae = " + associationEndId + "))"));
        if (result != null && result.length > 0 && !"".equals(result[0])) {
            return processQueryResult(intermediateDomain.query(result[0] + ".ejbAssociationEnd2"))[0];
        }
    ...
    private void transformUMLDataTypetoEJBDataType() throws Exception {
        ...
        intermediateDomain.insertUMLDataTypeToEJBDataType("UMLInteger", "EJBInteger");

        // "Double" -> "EJBDouble"
        targetDomain.insertEJBDataType("EJBDouble", "EJBDouble");
        intermediateDomain.insertUMLDataTypeToEJBDataType("UMLDouble", "EJBDouble");

        // "Real" -> "EJBDouble"
        intermediateDomain.insertUMLDataTypeToEJBDataType("UMLReal", "EJBDouble");

        // "String" -> "EJBString"
        targetDomain.insertEJBDataType("EJBString", "EJBString");
        intermediateDomain.insertUMLDataTypeToEJBDataType("UMLString", "EJBString");

        // "Date" -> "EJBDate"
        targetDomain.insertEJBDataType("EJBDate", "EJBDate");
        intermediateDomain.insertUMLDataTypeToEJBDataType("UMLDate", "EJBDate");

        // "Boolean" -> "EJBBoolean"
        targetDomain.insertEJBDataType("EJBBoolean", "EJBBoolean");
        intermediateDomain.insertUMLDataTypeToEJBDataType("UMLBoolean", "EJBBoolean");
    }
    //--------------------------------------------------------------------------
    ...
    private void createEJBKeyClassfromUMLClass(String umlClassId, String umlClassName) throws Exception {
        ...
        intermediateDomain.insertUMLClassToEJBKeyClass(umlClassId, ejbKeyClassId, ejbAttributeId);
    }
    //--------------------------------------------------------------------------
    ...
    private void createEJBKeyClassfromUMLAssociationClass(String umlAssociationClassId, String umlAssociationClassName) throws Exception {
        ...
        intermediateDomain.insertUMLAssociationClassToEJBKeyClass(umlAssociationClassId, ejbKeyClassId, firstId, otherId);
    }
    //--------------------------------------------------------------------------
    // REGRA 3
    // Criacao
    ...
    private void createEjbEntityComponentfromUmlClass(String classId, String className) throws Exception {
        ...
        // Cria Juncao
        intermediateDomain.insertUMLClassToEJBEntityComponent(classId, ejbEntityComponentId, ejbDataClassId, ejbDataSchemaId, ejbServingAttributeId);
    }
    // Linkagem
    ...
    private void linkEjbEntityComponentfromUmlClass(String id) throws Exception {
        String umlClassId = processQueryResult(intermediateDomain.query(id + ".class"))[0];
        String ejbEntityComponentId = processQueryResult(intermediateDomain.query(id + ".entityComponent"))[0];
        String ejbDataClassId = processQueryResult(intermediateDomain.query(id + ".dataClass"))[0];
        String ejbDataSchemaId = processQueryResult(intermediateDomain.query(id + ".dataSchema"))[0];
        String ejbServingAttributeId = processQueryResult(intermediateDomain.query(id + ".servingAttribute"))[0];
...
//        String[] idsUmlOperations = tratarResultadoQuery(origem.query(idUmlClass + ".getAllContained().feature->select(f : Feature | f.oclIsKindOf(Operation))"));
        ...
        if (umlOperationsIds != null && umlOperationsIds.length > 0 && !"".equals(umlOperationsIds[0])) {
            for (String operationId : umlOperationsIds) {
                String businessMethodId = processQueryResult(intermediateDomain.query(operationId + ".transformerToBusinessMethod.businessMethod"))[0];
        ...
        if (umlAttributesIds != null && umlAttributesIds.length > 0 && !"".equals(umlAttributesIds[0])) {
            for (String attributeId : umlAttributesIds) {
                String ejbAttributeId = processQueryResult(intermediateDomain.query(attributeId + ".transformerToEjbAttribute.ejbAttribute"))[0];
    ...
    private void createEJBDataClassfromClass(String classId, String className) throws Exception {
        ...
        intermediateDomain.insertUMLClassToEJBDataClass(classId, ejbDataClassId);
    }
    // Linkagem
    ...
    private void linkEJBDataClassfromClass(String id) throws Exception {
        String umlClassId = processQueryResult(intermediateDomain.query(id + ".class"))[0];
        String ejbDataClassId = processQueryResult(intermediateDomain.query(id + ".ejbDataClass"))[0];
        ...
        String ejbDataSchemaId = processQueryResult(intermediateDomain.query("UMLClassToEJBEntityComponent.allInstances()->select(a | a.class->includes(" + umlClassId + ".getOuterMostContainer()))->collect(a | a.dataSchema)"))[0];
        targetDomain.insertEJBDataClassSchemaLink(ejbDataClassId, ejbDataSchemaId);
    }
    //--------------------------------------------------------------------------
    // REGRA 5
    ...
    private void createEJBDataAssociationfromAssociation(String id, String name) throws Exception {
        ...
        intermediateDomain.insertUMLAssociationToEJBDataAssociation(id, ejbDataAssociationId);
    }
    // Linkagem
    ...
    private void linkEJBDataAssociationfromAssociation(String id) throws Exception {
        String umlAssociationId = processQueryResult(intermediateDomain.query(id + ".association"))[0];
        String[] umlAssociationEndsIds = processQueryResult(sourceDomain.query(umlAssociationId + ".associationEnds"));

        String ejbDataAssociationId = processQueryResult(intermediateDomain.query(id + ".ejbDataAssociation"))[0];
        ...
        String ejbDataSchemaId = processQueryResult(intermediateDomain.query("UMLClassToEJBEntityComponent.allInstances()->select(a | a.class->includes(" + umlAssociationId + ".getOuterMostContainerFromAssociation())).dataSchema"))[0];
    ...
    private void createEJBDataClassfromAssociationClass(String id, String name) throws Exception {
        ...
        intermediateDomain.insertUMLAssociationClassToEJBDataClass(id, ejbDataClassId);
    }
    // Linkagem
    ...
    private void linkEJBDataClassfromAssociationClass(String id) throws Exception {
        String umlAssociationClassId = processQueryResult(intermediateDomain.query(id + ".associationClass"))[0];
        String ejbDataClassId = processQueryResult(intermediateDomain.query(id + ".ejbDataClass"))[0];
        ...
        String ejbDataSchemaId = processQueryResult(intermediateDomain.query("UMLClassToEJBEntityComponent.allInstances()->select(a | a.class->includes(" + umlAssociationClassId + ".getOuterMostContainer())).dataSchema"))[0];
        targetDomain.insertEJBDataClassSchemaLink(ejbDataClassId, ejbDataSchemaId);
    }
    //--------------------------------------------------------------------------
    // REGRA 7
    ...
    private void createEJBAttributefromAttribute(String id, String name) throws Exception {
        ...
        intermediateDomain.insertUMLAttributeToEJBAttribute(id, ejbAttributeId);
    }
    // Linkagem
    ...
    private void linkEJBAttributefromAttribute(String id) throws Exception {
        String umlAttributeId = processQueryResult(intermediateDomain.query(id + ".attribute"))[0];
        String umlAttributeTypeId = processQueryResult(sourceDomain.query(umlAttributeId + ".classifier"))[0];

        String ejbAttributeId = processQueryResult(intermediateDomain.query(id + ".ejbAttribute"))[0];
    ...
    private void createEJBAssociationEndfromAssociationEndusingRule8(String id, String name) throws Exception {
        ...
        intermediateDomain.insertUMLAssociationEndToEJBDataEndusingRule8(id, ejbAssociationEndId);
    }
    // Linkagem
    ...
    private void linkEJBAssociationEndfromAssociationEndusingRule8(String id) throws Exception {
        String umlAssociationEndId = processQueryResult(intermediateDomain.query(id + ".associationEnd"))[0];
        String ejbAssociationEndId = processQueryResult(intermediateDomain.query(id + ".ejbAssociationEnd"))[0];
    ...
    private void createEJBAssociationEndfromAssociationEndusingRule9(String id, String name) throws Exception {
        ...
        intermediateDomain.insertUMLAssociationEndToEJBDataEndusingRule9(id, ejbAssociationEndId);
    }
    // Linkagem
    ...
    private void createEJBAssociationEndfromAssociationusingRule10(String id, String name) throws Exception {
        ...
        intermediateDomain.insertUMLAssociationEndEmEJBAssociationusingRule10(id, ejbAssociationId, ejbAssociationEnd1Id, ejbAssociationEnd2Id);
    }
    // Linkagem
    private void linkRule10UMLAssociationEndtoEJBAssociation() throws Exception {
        String[] ids = processQueryResult(intermediateDomain.query("UMLAssociationEndEmEJBAssociationusingRule10.allInstances()"));
    ...
    private void linkEJBAssociationEndfromAssociationusingRule10(String id) throws Exception {
        String umlAssociationEndId = processQueryResult(intermediateDomain.query(id + ".associationEnd"))[0];
        String ejbDataAssociationId = processQueryResult(intermediateDomain.query(id + ".ejbDataAssociation"))[0];
        String ejbAssociationEnd1Id = processQueryResult(intermediateDomain.query(id + ".ejbAssociationEnd1"))[0];
        String ejbAssociationEnd2Id = processQueryResult(intermediateDomain.query(id + ".ejbAssociationEnd2"))[0];
    ...
    private void createEJBAssociationEndfromAssociationusingRule11(String id, String name) throws Exception {
        ...
        intermediateDomain.insertUMLAssociationEndEmEJBAssociationusingRule11(id, ejbAssociationEnd2Id);
    }
    // Linkagem
    ...
    private void linkEJBAssociationEndfromAssociationusingRule11(String id) throws Exception {
        String umlAssociationEndId = processQueryResult(intermediateDomain.query(id + ".associationEnd"))[0];
        String ejbAssociationEndId = processQueryResult(intermediateDomain.query(id + ".ejbAssociationEnd2"))[0];
    ...
    private void createBusinessMethodfromOperation(String id, String name) throws Exception {
        ...
        intermediateDomain.insertUMLOperationToBusinessMethod(id, businessMethodId);
    }
    // Linkagem
    ...
    private void linkBusinessMethodfromOperation(String id) throws Exception {
        String umlOperationId = processQueryResult(intermediateDomain.query(id + ".operation"))[0];
        ...
        String businessMethodId = processQueryResult(intermediateDomain.query(id + ".businessMethod").replace("'", ""))[0];
    ...
    private void createEJBParameterfromParameter(String id, String name) throws Exception {
        ...
        intermediateDomain.insertUMLParameterToEJBParameter(id, ejbParameterId);
    }
    // Linkagem
    private void linkUMLParametertoEJBParameter() throws Exception {
        String[] ids = processQueryResult(intermediateDomain.query("UMLParameterToEJBParameter.allInstances()"));
        for (String id : ids) {
            if (id != null && !"".equals(id)) {
