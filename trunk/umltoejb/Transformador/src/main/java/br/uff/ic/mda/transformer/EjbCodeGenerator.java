...
//                    Atributo atrib = SintaxeJava.getJavaAttribute().setVisibilidade("private").setTipo(tipoLista).setNome(nomeCompleto);
                    ...
                    add.setName("add" + upperFirstLetter(simpleName)).setReturnType("void").setVisibility("public").addParameter(type, simpleName).setCode(new String[]{"if (" + simpleName + " == null) {", "\tthis." + simpleName + " = new ArrayList<" + type + ">();", "}", "this. " + simpleName + ".add(" + simpleName + ");"});
